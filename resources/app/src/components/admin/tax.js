...
export default class Tax extends React.Component {
    ...
    async handleQuery(taxSetting) {
      ...
      let query = `SELECT SUM(TaxPaid) as TaxPaid,SUM(AmountReceived) as TotalChargesPaid,FirstName,LastName from dbo.Payments Where FirstName='${FirstName}' AND LastName='${LastName}' AND Year(PaymentDate)=${Year} AND Month(PaymentDate) IN(${month1},${month2},${month3}) GROUP BY FirstName,LastName`;
